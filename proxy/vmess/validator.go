...
func (v *TimedUserValidator) generateNewHashesWithHash(nowSec protocol.Timestamp, user *user, userHash map[[16]byte]indexTimePair) {
	var hashValue [16]byte
	genEndSec := nowSec + cacheDurationSec
	genHashForID := func(id *protocol.ID) {
		idHash := v.hasher(id.Bytes())
		...
		for ts := genBeginSec; ts <= genEndSec; ts++ {
			...
			userHash[hashValue] = indexTimePair{
				user:        user,
				timeInc:     uint32(ts - v.baseTime),
				taintedFuse: new(uint32),
			}
...
func (v *TimedUserValidator) ResetUsers(users []*protocol.MemoryUser) error {
	newUsers := make([]*user, 0, 16)
	newUserHash := make(map[[16]byte]indexTimePair, 1024)
	...
	for _, item := range users {
		...
		newUsers = append(newUsers, uu)
		v.generateNewHashesWithHash(protocol.Timestamp(nowSec), uu, newUserHash)
		...
		v.aeadDecoderHolder.AddUser(cmdkeyfl, item)
	}
	v.Lock()
	defer v.Unlock()
	v.users = newUsers
	v.userHash = newUserHash
	return nil
}
