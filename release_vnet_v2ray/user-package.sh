...
#	fi
  ...
  local VERSIONTAG=$(git describe --abbrev=0 --tags)
	...
	echo ">>> Compile v2ray ..."
	env CGO_ENABLED=0 go build -o "$TMP"/vnet-v2ray"${EXESUFFIX}" -ldflags "$LDFLAGS" ./vnet
	if [[ $GOOS == "windows" ]]; then
		env CGO_ENABLED=0 go build -o "$TMP"/vnet-v2ray"${EXESUFFIX}" -ldflags "-H windowsgui $LDFLAGS" ./vnet
	fi
