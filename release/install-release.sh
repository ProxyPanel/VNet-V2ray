...
install_startup_service_file() {
  install -m 644 "${TMP_DIRECTORY}/systemd/system/v2ray.service" /etc/systemd/system/vnet-v2ray.service
  ...
  if [[ -n "$JSONS_PATH" ]]; then
    "rm" '/etc/systemd/system/vnet-v2ray.service.d/10-donot_touch_single_conf.conf' \
...
ExecStart=/usr/local/bin/v2ray -confdir $JSONS_PATH" |
      tee '/etc/systemd/system/vnet-v2ray.service.d/10-donot_touch_multi_conf.conf' > \
...
ExecStart=/usr/local/bin/v2ray -config ${JSON_PATH}/config.json" > \
      '/etc/systemd/system/vnet-v2ray.service.d/10-donot_touch_single_conf.conf'
    echo "# In case you have a good reason to do so, duplicate this file in the same directory and make your customizes there.
# Or all changes you made will be lost!  # Refer: https://www.freedesktop.org/software/systemd/man/systemd.unit.html
[Service]
ExecStart=
