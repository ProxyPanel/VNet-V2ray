...
func init() {
	common.Must(common.RegisterConfig((*Config)(nil), func(ctx context.Context, config interface{}) (interface{}, error) {
		...
		d.Context = ctx
		if err := core.RequireFeatures(ctx, func(om outbound.Manager, router routing.Router, pm policy.Manager, sm stats.Manager, tm traffic.Manager) error {
			return d.Init(config.(*Config), om, router, pm, sm, tm)
		}); err != nil {
			return nil, err
		}
		return d, nil
...
func (d *DefaultDispatcher) Init(config *Config, om outbound.Manager, router routing.Router, pm policy.Manager, sm stats.Manager, tm traffic.Manager) error {
	d.ohm = om
	d.router = router
	d.policy = pm
	d.stats = sm
...
func (d *DefaultDispatcher) getLink(ctx context.Context) (*transport.Link, *transport.Link) {
	...
	repoter := core.MustFromContext(d.Context).GetFeature(online.Type()).(*online.OnlineRepoter)
	if repoter != nil {
		ib := session.InboundFromContext(ctx)
		if ib != nil && user != nil && len(user.Email) > 0 {
			repoter.Online(user.Email, ib.Source.Address.IP().String())
		}
	}
...
func (d *DefaultDispatcher) Dispatch(ctx context.Context, destination net.Destination) (*transport.Link, error) {
	...
	ruleManager := core.MustFromContext(d.Context).GetFeature(rule.Type()).(*rule.RuleManager)
	if ruleManager != nil {
		if !ruleManager.Do(ctx, destination) {
